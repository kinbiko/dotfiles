...
code_map("r", function() Snacks.picker.lsp_references() end, "References")
code_map("i", function() Snacks.picker.lsp_implementations() end, "Implementations")
code_map("t", function() Snacks.picker.lsp_type_definitions() end, "Type definition")
code_map("s", function() Snacks.picker.lsp_symbols() end, "Document symbols")
code_map("S", function() Snacks.picker.lsp_workspace_symbols() end, "Workspace symbols")
...
code_map("E", function() Snacks.picker.diagnostics() end, "All diagnostics")

-- Refactor
code_map("n", vim.lsp.buf.rename, "Rename symbol")
code_map("a", vim.lsp.buf.code_action, "Code action", { "n", "x" })
code_map("f", function() require("conform").format({ async = true, lsp_format = "fallback" }) end, "Format", { "n", "x" })
...
local function claude_refactor_selection()
  ...
  vim.ui.input({ prompt = "Refactor instruction: ", default = "Refactor this code for clarity" }, function(instruction)
...
-- Lint
code_map("l", function() require("lint").try_lint() end, "Re-lint buffer")
...
local function parse_failure_location(output_lines)
  for _, line in ipairs(output_lines or {}) do
    ...
    if file then return file, tonumber(lnum), msg end
  end
end
...
test_map("p", function() go_test() end, "Run tests in package")
...
local function show_coverage_picker()
  ...
  Snacks.picker.pick({
    ...
    format = function(item)
      local hl = item.percent >= 80 and "DiagnosticOk" or (item.percent >= 50 and "DiagnosticWarn" or "DiagnosticError")
      return {
...
require("lazy").setup({
    ...
    keys = {
      ...
      { "<leader>fr", function() Snacks.rename.rename_file() end, desc = "Rename current file" },
      {
        ...
        function()
          ...
          if ext == "" or stem == "" then return end
          ...
          elseif stem:match("%.spec$") then
            ...
            is_test, alt_stems = false, { stem .. "_test", "test_" .. stem, stem .. ".test", stem .. ".spec" }
          end
      ...
      { "<leader>go", function() Snacks.gitbrowse.open({ what = "permalink" }) end, desc = "Open on git forge (permalink)", mode = { "n", "x" } },
  ...
  },
  {
    ...
    config = function()
      ...
      require("nvim-treesitter").install({
        ...
        "bash", "go", "rust", "python", "javascript", "typescript", "tsx",
        "json", "yaml", "toml",
        "markdown", "markdown_inline",
        "hcl", "dockerfile", "html", "css",
        "gitcommit", "diff", "regex",
      })
      vim.api.nvim_create_autocmd("FileType", {
        callback = function(args)
  ...
  },
  {
    ...
    config = function()
      ...
      require("mason-lspconfig").setup({
        ensure_installed = {
          "gopls", "lua_ls", "terraformls", "rust_analyzer",
          "basedpyright", "ts_ls", "bashls", "jsonls", "yamlls",
        },
  ...
  },
  {
    ...
    opts = {
      ensure_installed = {
        "goimports", "stylua", "prettierd", "ruff",
        "shfmt", "shellcheck", "eslint_d", "golangci-lint",
      },
    },
  },
  {
    ...
    keys = {
      {
        ...
        function()
          ...
          vim.system({ "go", "test", "-coverprofile=" .. go_coverage_file, go_pkg_pattern() }, { text = true }, function(obj)
            vim.schedule(function()
              if obj.code ~= 0 then
                vim.notify("go test (coverage) failed:\n" .. (obj.stderr or ""), vim.log.levels.ERROR)
                return
              end
              coverage.load(true)
